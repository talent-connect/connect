  ...
  await from(allConProfiles)
    .pipe(
        ...
        (p) =>
          from(
            conn.sobject('Contact').create({
              ...
              LastName: `${p.lastName}`,
              redi_Contact_Gender__c: p.gender
            ...
            retryWhen((errors) =>
              errors.pipe(delay(10000), take(30), concatMap(throwError))
            )
          ),
        (p, { id }) => {
          return { ...p, sfContactId: id }
        },
        10
      ),
      ...
      mergeMap(
        (p) =>
          from(
            conn.sobject('ReDI_Connect_Profile__c').create({
              Contact__c: p.sfContactId,
              RecordTypeId:
                p.userType.indexOf('mentor') !== -1 ? MENTOR : MENTEE,
        ...
        (p, { id }) => {
          return { ...p, sfConnectProfileId: id }
        },
        10
      ),
  ...
  await from(allConMentoringSessions)
    .pipe(
      mergeMap(
        (p) =>
          from(
            conn.sobject('Mentoring_Session__c').create({
              Date__c: p.date,
