...
module.exports = function (RedProfile) {
  ...
  RedProfile.observe('loaded', (ctx, next) => {
    ...
    const isRedProfileOwnedByCurrentUser =
      currentUserRedProfileId && currentUserRedProfileId === ctx.data.id.toString();
    ...
    if (ctx.data && ctx.data.userType === 'mentor') {
      ...
      const getRedMentoringSessionsToCurrentMentor = () =>
        ctx.options.currentUser
          ? Rx.bindNodeCallback(RedMentoringSession.find.bind(RedMentoringSession))({
              where: {
                menteeId: ctx.options.currentUser.redProfile.id,
                mentorId: ctx.data.id,
              },
  ...
  const pendingReviewAcceptOrDecline = (acceptDecline) => async (data, options, callback) => {
    ...
    const setNewRedProfileProperties = switchMap((redProfileInst) =>
      loopbackModelMethodToObservable(
        redProfileInst,
        'updateAttributes'
      )(currentUserTypeToPostReviewUpdates[acceptDecline][redProfileInst.toJSON().userType])
    );
...
const loopbackModelMethodToObservable = (loopbackModel, modelMethod) => (methodParameter) =>
  Rx.bindNodeCallback(loopbackModel[modelMethod].bind(loopbackModel))(methodParameter);
