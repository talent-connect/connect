...
if (!process.env.REDI_LOCATION) {
  throw new Error(
    'The environment variable REDI_LOCATION must be set! Set it to berlin or munich, or whatever else location is valid'
  );
}
  ...
  fp.map(({ name, surname, gender }) => {
    ...
    return {
      ...
      redProfile: {
        ...
        firstName: name,
        lastName: surname,
        birthDate: moment('01.01.' + ((new Date()).getFullYear() - _.random(18, 100, false)), 'DD.MM.YYYY'),
        ...
        languages: Languages.filter(() => Math.random() > 0.5).concat('English'),
        ...
        mentee_currentlyEnrolledInCourse: courses[Math.floor(Math.random() * courses.length)].id,
      },
    };
  })
)(persons);
...
const redProfileDestroyAll = Rx.bindNodeCallback(RedProfile.destroyAll.bind(RedProfile));
const redMatchDestroyAll = Rx.bindNodeCallback(RedMatch.destroyAll.bind(RedMatch));
const redMentoringSessionDestroyAll = Rx.bindNodeCallback(
  RedMentoringSession.destroyAll.bind(RedMentoringSession)
);

const redMatchCreate = (redMatch) => Rx.bindNodeCallback(RedMatch.create.bind(RedMatch))(redMatch);
const redUserCreate = (redUser) => Rx.bindNodeCallback(RedUser.create.bind(RedUser))(redUser);
const redProfileCreateOnRedUser = (redUserInst) => (redProfile) =>
  Rx.bindNodeCallback(redUserInst.redProfile.create.bind(redUserInst))(redProfile);
...
Rx.of({})
  .pipe(
    ...
    switchMap((redUser) => redProfileCreateOnRedUser(redUser)(ericAdminRedProfile)),
    switchMap(() => redUserCreate(ericMenteeRedUser)),
    switchMap((redUser) => redProfileCreateOnRedUser(redUser)(ericMenteeRedProfile)),
    switchMap(() => redUserCreate(ericMentorRedUser)),
    tap(console.log),
    switchMap((redUser) => redProfileCreateOnRedUser(redUser)(ericMentorRedProfile)),
    switchMap(() => redUserCreate(isabelleMentorRedUser)),
    tap(console.log),
    switchMap((redUser) => redProfileCreateOnRedUser(redUser)(isabelleMentorRedProfile)),
    tap(console.log),
    switchMapTo(users),
    concatMap(
    ...
    concatMap(
      (userData) => redProfileCreateOnRedUser(userData.redUserInst)(userData.redProfile),
      (userData, redProfileInst) => ({ ...userData, redProfileInst })
    ),
    toArray(),
    // Pick X mentor-mentee pairs, create match
    switchMap((data) => {
      const mentors = data.filter((userData) => userData.redProfile.userType === 'mentor');
      const mentees = data.filter((userData) => userData.redProfile.userType === 'mentee');
      ...
      for (let i = 0; i < locations.length; i++) {
        ...
        const matches = mentorsInLocation.map((mentor) => {
          return _.sampleSize(menteesInLocation, Math.floor(Math.random() * 10)).map((mentee) => {
            console.log(location);
            return {
              rediLocation: '' + location + '',
              applicationText: randomString(),
              expectationText: randomString(),
              status: ['applied', 'accepted', 'completed'][Math.floor(Math.random() * 3)],
