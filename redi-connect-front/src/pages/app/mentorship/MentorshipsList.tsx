...
function MentorshipList({ matches }: Props) {
  if (matches.length === 1) return <Redirect to={`/app/mentorships/${matches[0].id}`} />

  const subHeading = matches.length === 0
  ...
  return (
    <LoggedIn>
      ...
      <Content italic={matches.length === 0} size="medium" className="double-bs" renderAs="p" responsive={{ mobile: { hide: { value: true } } }}>
        {subHeading}
      </Content>
      <Columns>
